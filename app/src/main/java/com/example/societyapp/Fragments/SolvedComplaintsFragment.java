...
import com.example.societyapp.Adapters.SolvedComplaintAdapter;
...
public class SolvedComplaintsFragment extends Fragment {
    RecyclerView solvedcomplainRv;
    SolvedComplaintAdapter solvedComplaintAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.solved_complaint, container, false);
        solvedcomplainRv = rootView.findViewById(R.id.solvedcomplainRv);
    ...
    public void getSolvedList(){

        progressDialog.show();
        progressDialog.setMessage("Loading...");
        progressDialog.setCanceledOnTouchOutside(false);

        String url = "https://ivrics.com/societyAPI/index.php/Complaint/SolvedComplaint?id="+user_id;
                    ...
                    if(status_str.equals("success") && jsonObject!=null) {
                        ...
                        if (array.length() > 0) {
                            for (int i = 0; i < array.length(); i++) {
                                ...
                                SolvedListModel item = new SolvedListModel(
                                        cid, uid, subject, complaint, createdat, name,flatno);

                                solvedListModels.add(item);

                            }
                        }  else {
                            Toast.makeText(getActivity(), "No Data Found",
                                    Toast.LENGTH_LONG).show();
                            solvedcomplainRv.setVisibility(View.GONE);
                            nodataTv.setVisibility(View.VISIBLE);
                        }
                    }

                    solvedComplaintAdapter = new SolvedComplaintAdapter(solvedListModels, getActivity());
                    solvedcomplainRv.setAdapter(solvedComplaintAdapter);
